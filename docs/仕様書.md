...
interface LogEntry {
  ...
  centDeviations: (number | null)[]; // セント単位のズレ
  settings: {
    a4Freq: number;
...
| ファイルパス                             | 役割                             |
| ---------------------------------------- | -------------------------------- |
| `app/experiment/page.tsx`                | 実験ページ（ログ機能統合）       |
| `lib/hooks/useLogRecorder.ts`            | ログ記録フック                   |
| `lib/utils/exportLog.ts`                 | CSVエクスポート処理              |
| `components/feature/LogExportButton.tsx` | ログコントロールUIコンポーネント |
| `lib/audio_analysis/justAnalyze.ts`      | セント誤差の直接計算             |
