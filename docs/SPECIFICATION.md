...
graph TD
    ...
    subgraph NextApp [フロントエンドアプリケーション]
        C --> E[📄 UIコンポーネント層];
        E --> F[⚙️ ビジネスロジック層];
...
pnpm workspaces による monorepo 構成（詳細は [ARCHITECTURE.md](./ARCHITECTURE.md) 4章を参照）。

```
chordlens/                    # プロジェクトルート (monorepo)
...
│   └── __tests__/           # コアロジックのテスト (Vitest, Node)
├── apps/web/                # @chordlens/web: Web アプリ
...
└── package.json             # ルートscripts（各workspaceへ委譲）
```
...
- 実装詳細
    - **推定アルゴリズム**: `estimateRoot()` 関数（`packages/core/src/audio_analysis/rootEstimation.ts`）
...
- 実装詳細
    ...
    - **計算ロジック**: `getEqualJustDiff()` 関数（`packages/core/src/audio_analysis/calcJustFreq.ts`）
...
- **関連コンポーネント**:
    ...
    - `apps/web/src/App.tsx`: プリセット機能の統合（✅実装済み）
    - `app/experiment/page.tsx`: 実験ページでもプリセット機能を使用（✅実装済み）
    - `apps/web/lib/presets.ts`: プリセット関連のユーティリティ関数（✅実装済み）
    - `apps/web/lib/hooks/usePitchList.ts`: プリセット読み込みロジックを統合（✅実装済み）
...
**実装済み**: `packages/core/src/types.ts`に完全な型定義あり
...
| `apps/web/lib/hooks/useLogRecorder.ts`            | ログ記録フック                   | ✅実装済み |
| `apps/web/lib/utils/exportLog.ts`                 | CSVエクスポート処理              | ✅実装済み |
| `components/feature/LogExportButton.tsx` | ログコントロールUIコンポーネント | ✅実装済み |
| `packages/core/src/audio_analysis/justAnalyze.ts`      | セント誤差の直接計算             | ✅実装済み |
| `apps/web/lib/hooks/useAudioAnalysis.ts`          | centDeviations配列の提供         | ✅実装済み |
...
- **Vite 静的ビルド**: 事前ビルドされた SPA による高速な初期読み込み
- **コード分割**: 自動的なチャンク分割（Vite/Rollup機能）
...
- **モジュール化設計**:
    ...
    - 音声解析ロジック: `packages/core/src/audio_analysis/`
    - 定数・スキーマ: `packages/core/src/constants.ts`, `packages/core/src/types.ts`
- **依存関係の分離**:
...
いずれもリポジトリルートで実行できる（各 workspace へ委譲される）。

| コマンド            | 説明                              |
| ------------------- | --------------------------------- |
| `pnpm dev`          | 開発サーバー起動（Vite）          |
| `pnpm build`        | 本番ビルド（静的 SPA 生成）       |
| `pnpm preview`      | ビルド結果のプレビュー            |
| `pnpm lint`         | ESLint実行                        |
| `pnpm typecheck`    | 全 workspace の型チェック         |
| `pnpm test`         | 全 workspace の Vitest テスト実行 |
| `pnpm test:watch`   | Vitest ウォッチモード（apps/web） |
| `pnpm analyze:file` | 音声ファイルのオフライン解析 CLI  |

## 7.3. ファイル編集ガイド

### 7.3.1. 新しい音名を追加する場合

1. **`packages/core/src/constants.ts`を編集**:
...
`packages/core/src/constants.ts`のCHORD_DEFINITIONSを編集:
...
1. **`apps/web/src/App.tsx`にStateを追加**:
...
- **Code Splitting**: Vite/Rollupの自動分割を活用
